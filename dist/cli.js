...
nordnet.login(username, password).then(async () => {
    ...
    const chart = asciichart.plot(perf.map((p) => p.accumulated_result.value), {
        height: 20,
    ...
    const accounts = await Promise.all(accountResult.map(async (account) => {
        const accountResult = await nordnet.get_account(account.accid);
        return accountResult;
    }));
    const totalMoney = accounts.reduce((acc, a) => acc + a.full_marketvalue.value, 0);
